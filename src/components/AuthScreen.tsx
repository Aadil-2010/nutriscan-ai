...
import { ShieldCheck, Stethoscope, HeartPulse, User, Mail, ArrowRight, Lock } from 'lucide-react';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLoginSuccess }) => {
  ...
  // 1. Google OAuth / One-Click Fast Login
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      const googleProfile: UserProfile = {
        ...
        name: 'Google User',
        isLoggedIn: true,
        symptoms: 'Mild dairy & sulfite sensitivity',
        medicalReports: [
          {
            id: 'rep-init-1',
            title: 'Verified Baseline Health Profile',
            category: 'Allergy Panel',
            reportDate: new Date().toISOString().split('T')[0],
            reportText: 'No severe acute allergies reported. Monitoring dietary additives and food preservatives.',
      ...
      // Save directly to localStorage so reload never kicks you out
      localStorage.setItem('nutriscan_ai_user_profile_v1', JSON.stringify(googleProfile));
      onLoginSuccess(googleProfile);
      setIsLoading(false);
    }, 400);
  };

  // 2. Email & Symptom Intake Form Submission
  const handleEmailLogin = (e: React.FormEvent) => {
    ...
    const profile: UserProfile = {
      ...
      name: name.trim(),
      isLoggedIn: true,
      symptoms: symptoms.trim() || 'None recorded',
      medicalReports: [],
      medicalReportAnalysis: undefined,
    };
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">
            AI-Powered Food Additive, Allergen & Medical Profile Screener
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-slate-500 font-medium uppercase">Or enter details</span>
          <div className="flex-1 h-px bg-slate-800" />
        </div>

        {/* Email / Manual Intake Form */}
        <form onSubmit={handleEmailLogin} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-300 mb-1.5">
              Known Symptoms / Allergies (Optional)
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="e.g., Lactose intolerant, asthma with sulfites, peanut allergy"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
              />
            </div>
          </div>
          ...
          >
            <span>Start Health & Food Screening</span>
        ...
        <div className="flex items-center justify-center gap-1.5 pt-2 text-[10px] text-slate-500 text-center">
          ...
          <span>Encrypted patient session • FSSAI, FDA & EFSA Standards</span>
        </div>
