...
export const CameraModal: React.FC<CameraModalProps> = ({
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      if (mode === 'barcode') {
        setScanMessage('Point laser at barcode...');
        try {
          codeReader = new BrowserMultiFormatReader();
          await codeReader.decodeFromVideoDevice(
            undefined, // Automatically uses default rear camera
            videoRef.current!,
            (result) => {
              if (result) {
          ...
          console.error('ZXing barcode scanner error:', err);
          setCameraError('Camera access blocked or device not supported.');
        }
      } else {
        // Label Mode: Standard camera stream for manual photo shutter button
        try {
          ...
          setScanMessage('Center food ingredient list inside frame');
        } catch (err: any) {
          console.error('Label camera error:', err);
          setCameraError('Camera access blocked. Please grant browser permissions.');
        }
      }
    };
        ...
        <div className="p-4 space-y-4">
          {cameraError ? (
            ...
            >
              ...
              {/* MODE 1: RED LASER OVERLAY FOR BARCODES */}
              {mode === 'barcode' && (
              ...
              {/* MODE 2: RESTORED ORIGINAL CYAN OVERLAY FOR LABELS */}
              {mode === 'image' && (
